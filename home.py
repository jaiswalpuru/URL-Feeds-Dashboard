    ...
    if(request.method=='POST'):
        ...
        for tweets in account_info_json:
            ...
            if(len(video_links)!=0):
                twitter_names.append((tweets['user']['name']))
        ...
        with sql.connect("database.db")as con:
            ...
            for i in range(0,len(url_links_urls)):
                cur.execute("INSERT OR IGNORE INTO FEEDS(ID,twitter_names,timeStamps,url_links)VALUES(?,?,?,?)",(usernames[0],twitter_names[i],url_links_dates[i],url_links_urls[i]))
            con.commit()
